...
public:
  double getProfit(int R, int B) {
    ...
    double *last_line = new double[R+1];
    double *curr_line = new double[R+1];
    double expected = 0.0;

    for (int r=0; r<=R; r++) last_line[r] = static_cast<double>(r);
    for (int b=1; b<=B; b++) {
      curr_line[0] = 0;

      for (int r=1; r<=R; r++) {
        double eR = curr_line[r-1] + 1;
        double eB = last_line[r] - 1;
        curr_line[r] = max((eR*r + eB*b)/(r + b), 0.0);
      }
      if (b == B) expected = curr_line[R];
