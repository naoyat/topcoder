...
class TurntableService {
 public:
  int calculateTime(vector <string> favorites) {
	int n=sz(favorites);
    // ll ful = (1LL<<n)-1;
    vll fav(n,0), eaten(n,0);
    rep(i,n){
      vi f = map_atoi(split(favorites[i]));
      tr(f,it) fav[i] |= (1LL << *it);
    }
    //cout << "fav: " << fav << endl;
    set<pair<int,pair<int,pair<int,vll> > > > s;
    priority_queue<pair<int,pair<int,pair<int,vll> > > > pq; // -sc, ofs, lastcmd, eaten
    pq.push(make_pair(0,make_pair(0,make_pair(0,eaten))));
    while(!pq.empty()){
      if (found(s,pq.top())) { pq.pop(); continue; }

      cout << pq.top() << endl;
      int sc=-pq.top().first;
      int last=pq.top().second.first;
      int ofs=pq.top().second.second.first; if (ofs < 0) ofs += n;
      vll etn=pq.top().second.second.second;
      //printf("sc:%d, last:%d, ofs:%d, ...\n", sc,last,ofs);
      s.insert(pq.top());
      pq.pop();

      bool finis=true;
      // rep(i,n) if ((etn[i] & fav[i]) < fav[i]) finis=false;
      rep(i,n) if ((etn[i] & fav[i]) == 0) finis=false;
      if (finis) return sc;

      // no eat, turn right<1>
      int nsc = sc + ((last==1)? 1 : 2);
      pq.push(make_pair(-nsc,make_pair(1,make_pair(ofs+1,etn))));
      // no eat, turn left<2>
      nsc = sc + ((last==2)? 1 : 2);
      pq.push(make_pair(-nsc,make_pair(2,make_pair(ofs-1,etn))));
      ...
      pq.push(make_pair(-nsc,make_pair(0,make_pair(ofs,netn))));
    }
    return -1;
  }
};
