...
//typedef pair<i_i,i_i> rect;

class DonutsOnTheGridEasy {
  //  vector<string> gr;
  int R,C;
  /*
  bool isZeroShape(rect bounds) {
    rep(i,cadr(bounds)){ int r=caar(bounds)+i;
      rep(j,cddr(bounds)){ int c=cdar(bounds)+j;
        if (gr[r][c]=='.') return false;
    }
    return true;
  }
g  */
 ...
 public:
  int calc(vector<string> grid) {
    ...
    vector<vector<int> > isle(R,vector<int>(C,-1));
    int isle_id=0;
    rep(r,R)rep(c,C){ if(grid[r][c]=='0') continue;
      if (c>=1 && isle[r][c-1]>=0) {
        isle[r][c] = isle[r][c-1];
      } else {
        isle[r][c] = ++isle_id;
      ...
      if (r>=1 && isle[r-1][c]>=0) {
        eq.insert(cons(isle[r-1][c],isle[r][c]));
      }
    }
    vector<i_i> eqv(all(eq));
    reverse(all(eqv));
    //    pp(isle);
    tr(eqv,it){
      int l=cdr(*it), s=car(*it);
      rep(r,R)rep(c,C) if (isle[r][c]==l) isle[r][c]=s;
    }
    //printf("==>\n");
    //pp(isle);
    set<int> vd;
    rep(r,R){
      if(isle[r][0]>=0) vd.insert(isle[r][0]);
      if(isle[r][C-1]>=0) vd.insert(isle[r][C-1]);
    }
    rep(c,C){
      if(isle[0][c]>=0) vd.insert(isle[0][c]);
      if(isle[R-1][c]>=0) vd.insert(isle[R-1][c]);
    }
    tr(vd,it){
      rep(r,R)rep(c,C) if (isle[r][c]==*it) isle[r][c]=-1;
    }
    pp(isle);
    //    cout << "void:" << vd << endl;
    //cout << "eq:" << eqv << endl;
