...
// END CUT HERE
	vector<int> rev(M);
	for (int i=0;i<M;i++) rev[shuffle[i]] = i;

	int cri = cardsReceived.size();
	vector<int> cR(all(cardsReceived));
	vector<int> p(M,0);
	for (int i=1; i<=maxShuffles; i++) {
	  vector<int> cR_(all(cR));
	  for (int j=0;j<cri;j++) {
		cR[j] = rev[cR_[j]];
		p[cR[j]]++;
	  }
	...
	vector<double> rs(M);
	for (int i=0;i<M;i++) rs[i] = 1.0 / maxShuffles * p[i];
	sort(all(rs));
	reverse(all(rs));

	double rate = 0.0;
	for (int i=0;i<K;i++) rate += rs[i];
	return rate;
  }
