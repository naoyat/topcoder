...
#define foreach(it,v) for(__typeof((v).begin

bool c[10000005], u[10000005];

class SquareFreeNumbers {
 public:
  int getCount(long long mn, long long mx) {
    int sz = int(max-min+11);
    vector<int> sqfree(sz,1);
    for(ll s=2; s<=1000000; s++){
      ll sq=s*s;
      ll start = (min/sq) * sq;
      for( ll n=start; n<=max; n+=sq){
        if (n>=min) {
          sqfree[int(n-min)] = 0;
        }
      }
