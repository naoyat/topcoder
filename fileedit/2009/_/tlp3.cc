...
// END CUT HERE
	...
	long long c = 0LL;
	//printf("LOOP 1: len=1..k<%d>, besides h=%d,m=%d\n", k, h,m);
	...
	if (h > k) {
	  //printf("LOOP 2: r=1..k<%d>; c=%lld\n", k, c);
	  long long o = 0LL;
	  for (int r=1; r<=k; r++) {
		long long co = coeffs[r];
		/*
		printf("  r=%d, coeffs[%d]=%lld, o=%lld\n", r,r,co,o);
		printf("    (h:%d > k:%d) r:%d, o += %lld x (%lld - %lld) = %lld x %lld = %lld",
			   h,k,r,
			   co, geo(r,h-1), geo(r,k),
			   co, sub_(geo(r,h-1), geo(r,k)),
			   mul_(co, sub_(geo(r,h-1), geo(r,k)))
			   );
		*/
		if (co >= 0)
