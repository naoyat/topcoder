...
public:
  Stat(int passed_mask_i, int lapse_i) {
  ...
  bool operator<(const Stat& s) {
	if (passed_count < s.passed_count) true;
...
class CavePassage {
  ...
  int back(Stat *st, set<int> history)//int passed_mask, int lapse)
  {
	printf("back(%02x, %02x, %d) %d ;; min_time=%d\n",
		   st->rest_mask(), st->passed_mask, st->lapse, history.size(), min_time);
	/*
	*/
	//	if (history.size() > nn * 2) return -1;

	if (st->lapse > min_time) return -1;
	...
	*/
	...
	//if (found(history, st->passed_mask)) return -1;
	history.insert(st->passed_mask);

	priority_queue<Stat*> pq;
	
	for (int m=all_mask; m>=1; m--) {
	  if (possible[m] == true && ((m & st->passed_mask) == m)) {
		int passed_mask_later = st->passed_mask - m;
		if (!found(history, passed_mask_later))
		  pq.push( new Stat(passed_mask_later, st->lapse + slowests[m]) );
	  }
	}

	int lmin = INT_MAX;
	while (!pq.empty()) {
	  Stat* next_st = pq.top(); pq.pop();
	  // next_st->description();
	  if (next_st->lapse > min_time) break;
	  int l = go(next_st, history);
	  if (l > 0) lmin = min(l,lmin);
	  delete next_st;
	}
	return (lmin == INT_MAX)? -1 : lmin;
  }

  // int go(int rest_mask, int lapse)
  int go(Stat *st, set<int> history) //int passed_mask, int lapse)
  {
	int rest_mask = st->rest_mask(); //all_mask - passed_mask;
	/*
	printf("go(%02x, %02x, %d) %d ;; min_time=%d\n",
		   rest_mask, st->passed_mask, st->lapse, history.size(), min_time);
	*/
	//if (history.size() > nn * 2) return -1;
	...
	for (int m=all_mask; m>=1; m--) {
	  if (possible[m] == true && ((m & rest_mask) == m)) {
		int passed_mask_later = st->passed_mask + m;
		if (passed_mask_later == all_mask) {
		  // mが移動して終わりなら・・・backを呼ぶ必要がない
		  min_time = lmin = min(min_time, st->lapse + slowests[m]);
		  // return st->lapse;
		} else {
		  if (!found(history, passed_mask_later))
			if (st->lapse + slowests[m] < min_time)
			  pq.push( new Stat(passed_mask_later, st->lapse + slowests[m]) );
		}
	  }
	}
	...
	while (!pq.empty()) {
	  ...
	  // next_st->description();
	  if (next_st->lapse > min_time) break;
	  int l = back(next_st, history);//st->passed_mask, st->lapse);
	  if (l > 0) lmin = min(l,lmin);
	  delete next_st;
	}
	return (lmin == INT_MAX)? -1 : lmin;
