...
// END CUT HERE
  ...
  dp(long long v, int since, vector<long long> pat, long long limit)
  {
	...
	cout << "  ref: " << ref << ", min = " << min_v << " at " << min_at << endl;
	cout << "  rv: " << rv << endl;
	// printf("v=%lld >= ov[%d] = %lld\n", v, since, ov[since]);
	// cout << "ref: " << ref << endl;
	long long ov0 = ov[since];
	// printf("v = %lld, ov0 = ov[%d] = %lld\n", v, since, ov[since]);

	if (v % ov0 == 0) {
	  vector<long long> pat_(pat.begin(),pat.end());
	  pat_[since] += v / ov0;
	  return make_pair(v / ov0, pat_);
	}

	long long min_retv = min_v;//min(min_v, limit);
	bool found = false;
	for (long long rest=v%ov0,cnt=v/ov0; rest < v; rest+=ov0,cnt--) {
	  if (cnt > limit) continue;
	  printf("  calling dp, v = %lld, ov0 = %lld, min_v = %lld, cnt = %lld\n", v, ov0, min_v, cnt);
	  pair<long long, vector<long long> > r = dp(rest, max(since+1,max_under(rest)), pat, min_v - cnt);
	  ...
	  if (v <= min_retv) {
		copy(r.second.begin(),r.second.end(),rv.begin());
		rv[since] += cnt;
		cout << "  3) rv = " << rv << endl;
		min_retv = v;
		found = true;
	...
	if (found == true)
	  return make_pair(min_retv, rv);
	else
	  return make_pair(-1, rv);
	// long long num = v / ov0, mod = v % ov0;
	/*
	int i = 0;
	for (long long cnt=v/ov0,rest=v%ov0; i<min_v; i++,cnt--,rest+=ov0) {
	  printf(" calling dp(%lld, since %d, %lld+%lld) where min_v = %lld\n", rest, since+1, hand,cnt, min_v);
	  long long dpv = hand + dp(v-rest,since+1,cnt) + dp(rest, since+1, 0);
	  if (dpv < min_v) min_v = dpv;
	}
	*/
  }
