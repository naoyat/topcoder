...
public:
  int minimalTime(vector<int> travelersWeights,
	...
	vector<vector<bool> > ways(patterns);
	vector<vector<int> > distance(2);
	vector<vector<bool> > passed(2);

	for (int i=0; i<2; i++) {
	  distance[i].resize(patterns);
	  passed[i].resize(patterns);
	}

	for (int i=0; i<=all_mask; i++) {
	  ways[i].resize(patterns);
	  ways[i][i] = false;
	  printf("looping %d/%d x %d\n", i, all_mask, all_mask);
	  for (int j=i+1; j<=all_mask; j++) {
		// i < j
		if (i == (i & j)) {
		  ways[i][j] = ways[j][i] = possible[j - i];
		} else {
		  ways[i][j] = ways[j][i] = false;
		  // continue;
		}
	  }
	  distance[0][i] = distance[1][i] = INT_MAX;
	  passed[0][i] = passed[1][i] = false;
	}
	distance[0][0] = 0;
	passed[0][0] = true;
  bye:
