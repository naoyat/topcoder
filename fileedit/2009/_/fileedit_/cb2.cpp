...
public:
  bool visited[29][29];
  double turn(int n,double r,int x,int y) {
	...
	if (e_ > 0) {
	  visited[x][y] = true;
	  total += turn(n-1,r*e_,x+1,y);
	  visited[x][y] = false;
	}
	if (w_ > 0) {
	  visited[x][y] = true;
	  total += turn(n-1,r*w_,x-1,y);
	  visited[x][y] = false;
	}
	if (n_ > 0) {
	  visited[x][y] = true;
	  total += turn(n-1,r*n_,x,y-1);
	  visited[x][y] = false;
	}
	if (s_ > 0) {
	  visited[x][y] = true;
	  total += turn(n-1,r*s_,x,y+1);
	  visited[x][y] = false;
	}
	return total;
  }
