...
class TheLongPalindrome {
  ...
  **/
  ...
  long long geo(int r, int n) {
	...
	/*
	  printf("1 + %d + %d^2 + .. + %d^%d = [(%lld + m - 1) % m]/%lld = %lld / %lld (m=%lld)\n",
		   r, r, r,n,
		   fast_expt(r,n+1,m),denom,
		   numer,denom,
		   m);
	if (numer % denom != 0)
	  printf("!! geo error %lld/%lld ... %lld\n", numer,denom, numer % denom);
	*/
...
// END CUT HERE
	...
	for (int len=1; len<=h; len++) {
	  for (int i=1; i<=k; i++) expts[i] = mul_(expts[i], i);
	  // cout << "len=" << len << ", expts=" << expts << endl;
	  if (len <= k) {
		int k_ = len;
		long long o = 0LL;
		for (int j=1; j<=k_; j++)
		...
		// k < len
		long long o = 0LL;
		for (int j=1; j<=k; j++) o += mul_(coeffs[j], expts[j]);
		geo(k,len);
		c = (len == h && m == 1)? add_(c,o) : add_(c,o*2);
	  }
	}
