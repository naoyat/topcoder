...
class HealthFood {
 public:
  vector <int> selectMeals(vector <int> protein,
    ...
    rep(i,n){
      ll p=0,c=0,f=0,t=0;
      ll pmag=0,cmag=0,fmag=0,tmag=0;
      string dp = dietPlans[i]; // 0-4 [CcPpFfTt]
      int dpl=sz(dp);
      rep(k,dpl){
        ll mag=1LL << ((3-k)*11);
        switch(dp[k]){
          case 'C': if (!c) {cmag=mag; c=-1;} break;
          case 'c': if (!c) {cmag=mag; c=1;} break;
          case 'P': if (!p) {pmag=mag; p=-1;} break;
          case 'p': if (!p) {pmag=mag; p=1;} break;
          case 'F': if (!f) {fmag=mag; f=-1;} break;
          case 'f': if (!f) {fmag=mag; f=1;} break;
          case 'T': if (!t) {tmag=mag; t=-1;} break;
          case 't': if (!t) {tmag=mag; t=1;} break;
        }
      }
      ...
      rep(j,m){
        ll score = 0;
        if (c) {
          if (c>0) score += cmag*carbs[j];
          else score += cmag*(100-carbs[j]);
        }
        if (p) {
          if (p>0) score += pmag*protein[j];
          else score += pmag*(100-protein[j]);
        }
        if (f) {
          if (f>0) score += fmag*fat[j];
          else score += fmag*(100-fat[j]);
        }
        if (t) {
          int calories=9*fat[j] + 5*(protein[j] + carbs[j]); // 1400 < 2048
          if (t>0) score += tmag*calories;
          else score += tmag*(1400-calories);
        }
        pt[j] = make_pair(score,j);
      }
      sort(all(pt));
      // printf("ans[%d] from pt[]:", i); cout << pt << endl;
      ans[i] = pt[0].second;
    }
    return ans;
  }
