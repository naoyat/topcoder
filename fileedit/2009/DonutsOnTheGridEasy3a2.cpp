...
class DonutsOnTheGridEasy {
  ...
  int sub2(int r,int c,int h,int w){
    //    if (h<2 || w<2) return 0;
    if (!isZeroShape(r,c,h,w)) return 0;
    //    if (h-2>=2 && w-2>=2)
    return 1+sub(r+1,c+1,h-2,w-2);
    //    else
    //return 1;
  }
  ...
  int sub(int r,int c,int h,int w){
    ...
    int sc=0;
    sc = max(sc,sub2(r,c,h,w)); // no split
    FOR(j,1,h-1){ // -
      sc=max(sc, sub(r,c,j,w)+sub(r+j,c,h-j,w));
    }
    FOR(i,1,w-1){ // |
      sc=max(sc, sub(r,c,h,i)+sub(r,c+i,h,w-i));
    }
    //mp[key]=sc;
    mp[r][c][h][w] = sc;
    return sc;
 ...
 public:
  int calc(vector<string> grid) {
    ...
    //mp.clear();
    //rep(r,50)rep(c,50)rep(w,51)rep(h,51) mp[r][c][h][w]=-1;
