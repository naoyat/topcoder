...
class CavePassage {
  ...
  int back(int rest_mask, int passed_mask, set<int> passers_go, set<int> passers_back, int lapse)
  {
	printf("back(%02x, %02x, %d) ;; min_time=%d\n", rest_mask, passed_mask, lapse, min_time);
	//	cout << passers_back << endl;

    if (passers_back.find(passed_mask) != passers_back.end()) return -1;
    passers_back.insert(passed_mask);

	if (passed_mask == all_mask) {
	  min_time = min(min_time, lapse);
	  return lapse;
	}

	int lmin = INT_MAX;
	for (int m=all_mask; m>=1; m--) {
	  if (possible[m] == true && (m & passed_mask) == m) {
		int s = go(rest_mask + m, passed_mask - m, passers_go, passers_back, lapse + slowests[m]);
		if (s < 0) continue;
		lmin = min(lmin, s);
	  }
	}
  ...
  int go(int rest_mask, int passed_mask, set<int> passers_go, set<int> passers_back, int lapse)
  {
	printf("go(%02x, %02x, %d) ;; min_time=%d\n", rest_mask, passed_mask, lapse, min_time);
	//	cout << passers_go << endl;
    if (passers_go.find(rest_mask) != passers_go.end()) return -1;
    passers_go.insert(rest_mask);
	...
	for (int m=all_mask; m>=1; m--) {
	  if (possible[m] == true && ((m & rest_mask) == m)) {
		int s = back(rest_mask - m, passed_mask + m, passers_go, passers_back, lapse + slowests[m]);
		if (s < 0) continue;
		lmin = min(lmin, s);
	  }
	}
...
public:
  int minimalTime(vector<int> travelersWeights,
	...
	set<int> passers_go;
    set<int> passers_back;

	go(all_mask, 0, passers_go, passers_back, 0);
