...
// END CUT HERE
	...
	rep(fid,n) {
	  int duree = forms[fid];
	  int duree_ = duree % dayLength;
	  printf("#%d: %d ... ", fid, duree_);
	  if (duree_ == 0) {
		printf("  ZERO\n");
	  } else {
		// [0 .. openTime] + duree_ => [duree .. openTime + duree_]
		int b = duree_, e = (openTime + duree_) % dayLength;
		if (b < e) {
		  if (b <= openTime) {
			if (e <= openTime) {
			  printf("  <?> [0..%d(oT)] => %d..%d\n", openTime, b, e);
			} else {
			  printf("  [0..%d] => %d..%d(oT),", openTime-duree_,  b, openTime);
			  printf("  [%d..%d(oT)] => 0\n", openTime-duree_, openTime );
			}
		  } else {
			printf("  [0..%d(oT)] => 0\n", openTime);
		  }
		  ...
		  if (e <= openTime) {
			printf("  [0..%d] => 0,", dayLength-duree_);
			printf("  [%d..%d(oT)] => 0..%d\n", dayLength-duree_, openTime,  e);
		  } else {
			printf("  [0..%d] => 0,",  dayLength-duree_);
			printf("  [%d..%d] => 0..%d(oT),",
				   dayLength-duree_, dayLength+openTime-duree_,  openTime);
			printf("  [%d..%d(oT)] => 0\n",
				   dayLength+openTime-duree_, openTime);
		  }
		}
	  }
	}
...
// END CUT HERE
	...
	lapse_min = min(lapse_min,lapse);

	if (forms[n-1] % dayLength < openTime) {
	  lapse = 0, hour = dayLength - (forms[n-1] % dayLength);
	  for (int fid=n-1; fid>=0; fid--) { // form ID
		int duree = forms[fid];
		hour = (hour + duree) % dayLength;
		if (0 < hour && hour < closeTime) {
