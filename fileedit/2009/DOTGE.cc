...
class DonutsOnTheGridEasy {
  ...
  int calcdp(int l,int t,int w,int h){
    if (DP[l][t][w][h]!=-1) return DP[l][t][w][h];
    int &ret = DP[l][t][w][h];
    if (w<3||h<3) ret=0;
    else {
      int k=1;
      FOR(i,l,l+w-1) if (grid1[i][t]!='0' || grid1[i][t+h-1]!='0') k=0;
      FOR(i,t,t+h-1) if (grid1[l][i]!='0' || grid1[l+w-1][i]!='0') k=0;

      ret=k+calcdp(l+1,t+1,w-2,h-2);
      int tmp=calcdp(l,t+1,w,h-1);
      ret = max(ret,tmp);
      tmp = calcdp(l+1,t,w-1,h);
      ret = max(ret,tmp);
      tmp = calcdp(l,t,w-1,h);
      ret = max(ret,tmp);
      tmp = calcdp(l,t,w,h-1);
      ret = max(ret,tmp);
    }
    return ret;
  }
 ...
 public:
  int calc(vector<string> grid) {
    ...
    FOR(i,0,n-1) FOR(j,0,m-1) FOR(w,1,n) FOR(h,1,m) DP[i][j][w][h]=-1;
    grid1.clear();
    
    rep(i,sz(grid)) grid1.pb(grid[i]);
    
    rep(i,n-1) rep(j,m-1) RFOR(w,3,n) RFOR(h,3,m)
        if(i+w-1<n && j+h-1<m) {
          int tmp=calcdp(i,j,w,h);
