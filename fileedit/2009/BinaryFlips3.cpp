 ...
 public:
  int minimalMoves(int A, int B, int K) {
    ...
    while(!pq.empty()) {
      ...
      int di=-tp.first;//, di=sc/100001;
      int st=tp.second.second; //, di=-tp.first;
      if (st >= v[z]) continue;
      v[z]=st;
      if(di==0) {
        //printf("di=0 at v[z:%d]=st:%d\n", z,st);
        maxst = st; break;
      }
      //if(di==0) { stmin=min(st,stmin); continue; }
      //printf("%d) dist=%d, (%d+%d)\n", st, -di, z,o);

      int f=min(K,z), t=K-min(K,o); //if (f>t) swap(f,t);
      //if(f<=t) {
      ...
      for(int d=dmin; d<=dmax; d+=2) {
        ...
        //if(v[newz]>st+1) {
        //if(v[newz]>st) //==INT_MAX)
        //int newsc = newdi*100001 + st+1;
        pq.push(make_pair(-newdi,make_pair(newz,st+1)));
          //}
      }
      //}
    }
    //if (v[A]<INT_MAX) return v[A];
    //printf("maxst = %d\n", maxst);
    return maxst;
  }
};
