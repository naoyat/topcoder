...
class DigitsSwap {
 public:
  string maximalProduct(string x, string y, int swaps) {
    ...
    //
    int k=sz(x), ka=k;
    vector<bool> af(k,true);
    rep(i,k) if(x[i]==y[i]) af[i]=false,ka--;
    if (swaps >= ka){
      int d = swaps - ka;
      swaps = ka - (d&1);
    }
    int swaps_1 = swaps;
    cout << "sw-: " << swaps << endl;
    if(bx<by) {string t=x;x=y;y=t;}//bignum t=bx;bx=by;by=t;}//bx>=by
    
    bool z=true;
    rep(i,k){
      //      if(z && x[i]>y[i]){z=false;continue;}
      if(x[i]>y[i]){
        if(z){z=false;continue;}
        char t=x[i];x[i]=y[i];y[i]=t; swaps--;
        if (swaps==0) break;
      }
    }
    cout << "x: " << x << endl;
    cout << "y: " << y << endl;
    cout << "sw: " << swaps << ", k: " << k << ", ka: " << ka << endl;
