...
int keta(int n,int base){
  int k=0;
  while (n>0) { k++; n /= base; }
  return k;
}
vector<int> num2vec(int n,int base){
  int kt=keta(n,base);
  vector<int> d(kt,0);
  for (int i=kt-1;i>=0;i--) { d[i] = n % base; n /= base; }
  return d;
}
int vec2num(vector<int> v,int base){
  int n=0;
  tr(v,it) { n += *it; n *= base; }
  return n;
}
...
class TheSwap {
 public:
  int findMax(int n, int k) {
    int kt=keta(n,10);
    vector<int> nv = num2vec(n,10);
	printf("n=%d: %d keta;", n,kt); cout << nv << endl;
    sort(all(nv));
    do {
      cout << "- " << vec2num(nv,10) << endl;
