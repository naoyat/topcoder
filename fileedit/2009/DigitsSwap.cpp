...
// BEGIN CUT HERE
    printf("%d keta, swapped: %d / lim:%d, rest=%d \n", keta, swapped, lim, rest);
// END CUT HERE
    if (rest && (keta & 1)) {
...
// END CUT HERE
      ...
      bignum nextmax = bignum(x) * bignum(y);
      swap(x[last_at],y[last_at]);
...
int Test_(Case_<8>) {
	...
	string y = "337882913";
	int swaps = 4;
	string RetVal = "88578661588126537";
	return verify_case(RetVal, DigitsSwap().maximalProduct(x, y, swaps)); }
