 ...
 public:
  ...
  vector<string> placeCircles(vector<int> pointX, vector<int> pointY, int M) {
    ...
    rep(j,N){ // point[j]
      ...
      //centre[b].second = max(centre[b].second, dd_distance(point[j],centre[b].first));
      
      //cout << "x => " << centre[b] << endl;
      /*
      double new_r = dd_distance(point[j],centre[b].first);
      if (new_r > centre[b].second)
        centre[b] = make_pair(centre[b].first, new_r);
      */
    }
      // cout << "centre: " << centre << endl;
