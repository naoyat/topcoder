...
class BaronsAndCoins {
  vector<i_i> p;
  vector<vector<i_i> > tt;
  vector<map<int,int> > score;

  int sub(int j, int last_dxmax) {
    int saved_max=-1;
    tr(score[j],it){
      if (it->first < last_dxmax) {
        if (it->second > saved_max) {
// BEGIN CUT HERE
          printf(" // score[j=%d<%d,%d>][%d<%d] = %d\n",
                 j, car(p[j]),cdr(p[j]),
                 it->first, last_dxmax,
                 it->second );
// END CUT HERE
    ...
    if (saved_max >= 0) return saved_max;

    int smax=-1, at_dxmax_=-1;
    FORR(i,0,j-1){
      i_i dif = tt[i][j];
      int dx=car(dif),dy=cdr(dif); if (dy==0) continue;
      int dxmax = (dx - (dy*(dy-1)/2))/dy;
      if (dxmax <= 0) continue;
      int dxmax_ = dxmax + (dy-1) + ((dx - (dy*(dy-1)/2) - dxmax*dy)?1:0);
// BEGIN CUT HERE
      printf(" - testing %d(%d,%d) - %d(%d,%d), diff=(%d,%d), %d..{%d-(%d+%d)=%d}..%d|%d\n",
             i, car(p[i]), cdr(p[i]),
             j, car(p[j]), cdr(p[j]),
             dx, dy,
             dxmax, dxmax_,
...
// END CUT HERE
      ...
      if (dxmax_ < last_dxmax) {
        int s = (i==0)? 1 : (sub(i,dxmax)+1);
        if (s > smax) { smax = s; at_dxmax_ = dxmax_; }
      }
    }
    // score[j][smax_dx] = smax;
    if (smax >= 0) {
      score[j][at_dxmax_] = smax;
      printf(" // score[j=%d][%d] = %d\n", j, at_dxmax_, smax);
      printf("sub(j=%d <%d,%d>, %d) = %d\n",
             j, car(p[j]), cdr(p[j]),
             last_dxmax, smax);
      return smax;
    } else {
 ...
 public:
  int getMaximum(vector <int> x, vector <int> y) {
    ...
    tt.resize(N); rep(i,N) tt[i].resize(N);
    score.resize(N); rep(i,N) score[i].clear();
