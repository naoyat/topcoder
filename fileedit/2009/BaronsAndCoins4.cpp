...
class BaronsAndCoins {
  ...
  int sb(int j, int last_dxmax) {
    int saved_max=-1;
    tr(score[j],it){
      if (it->first >= last_dxmax) continue;
      if (it->second > saved_max) saved_max = it->second;
    }
    if (saved_max >= 0) return saved_max;

    int smax=0, smax_dx=-1;
    forrr(i,0,j-1){
      i_i dif = tt[i][j];
...
// BEGIN CUT HERE
      printf("testing %d-%d, diff=(%d,%d), dxmax=%d..%d{%d-(%d+%d)=%d}|%d,..\n",
             i,j, dx,dy,
             dxmax, dxmax_,
...
// END CUT HERE
      ...
      int s = (i==0)? 1 : sb(i,dxmax)+1;
      if (s > smax) { smax = s; smax_dx = dxmax_; }
    }
