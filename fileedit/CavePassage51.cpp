...
public:
  int minimalTime(vector<int> travelersWeights,
	...
	*/
	...
	tr(possibles,it) {
	  int m = *it;
	  distance_to_fix[AWAY][m] = minute[m];
	}

	while (1) { // while (distance_fixed[AWAY][goal] == INT_MAX) {
	  ...
	  for (int j=1; j<=all_mask; j++) {
		// away side
		if (distance_to_fix[AWAY][j] == distance_min) {
		  distance_fixed[AWAY][j] = distance_min;
		  distance_to_fix[AWAY][j] = INT_MAX;
		  if (j == goal) return distance_min;
		  ...
		  tr(possibles,it) {
			...
			if ((j & m) == m) {
			  ...
			  int home_at = j - m; // 戻す
			  if (home_at == start || home_at == goal) continue;
			  if (distance_fixed[HOME][home_at] == INT_MAX)
				if (d < distance_to_fix[HOME][home_at]) distance_to_fix[HOME][home_at] = d;
			}
		  }
		}
		// home side
		if (distance_to_fix[HOME][j] == distance_min) {
		  distance_fixed[HOME][j] = distance_min;
		  distance_to_fix[HOME][j] = INT_MAX;
		  ...
		  tr(possibles,it) {
			...
			if ((j & m) == 0) {
			  int d = distance_min + minute[m];
			  int away_at = j + m; // 進める
			  // if (away_at == start) continue;
			  if (distance_fixed[AWAY][away_at] == INT_MAX)
				if (d < distance_to_fix[AWAY][away_at]) distance_to_fix[AWAY][away_at] = d;
			}
		  }
		}
