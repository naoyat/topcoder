...
class MessageMess {
  ...
  map<pair<string,string>,pair<int,string> > memo;
  set<string> visited;

  pair<int,string> sub(string msg,string curr){
    if (msg == "") return cons(1,curr);

    if (found(visited,msg)) return cons(2,"");
    visited.insert(msg);
    ...
    rep(i,n){
      ...
      switch(car(subres)) {
        ...
        default: return memo[key] = cons(2,"");
      }
    }

    switch(sz(res)){
      case 0: return memo[key] = cons(0,"");
      case 1: return memo[key] = cons(1,res[0]);
      default: return memo[key] = cons(2,"");
    }
  }
