...
public:
  ...
  double turn(int n,double r,int x,int y,vector<vector<bool> > h) {
	// printf("turn(%d, %g, %d,%d)\n", n,r,x,y);
	// cout << h << endl;

	if (h[14+x][14+y]) return 0.0;
	//	if (h.find(make_pair(x,y)) != h.end()) return 0.0;
	if (n == 0) return r;
	...
	double total = 0.0;
	if (e_ > 0) {
	  total += turn(n-1,r*e_*0.01,x+1,y,h);
	}
	if (w_ > 0) {
	  total += turn(n-1,r*w_*0.01,x-1,y,h);
	}
	if (n_ > 0) {
	  total += turn(n-1,r*n_*0.01,x,y-1,h);
	}
	if (s_ > 0) {
	  total += turn(n-1,r*s_*0.01,x,y+1,h);
	}
	return total;
  }
  double getProbability(int n, int east, int west, int south, int north) {
	e_=east; w_=west; s_=south; n_=north;
	printf("%d, E:%d W:%d S:%d N:%d\n", n, e_,w_,s_,n_);
	double rate = 1.0;
	
 	//set<pair<int,int> > h;
 	vector<vector<bool> > h(29);
	for(int i=0;i<29;i++) {
	  h[i].resize(29);
	  for(int j=0;j<29;j++) {
		h[i][j] = false;
