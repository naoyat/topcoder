...
class JumpingBoard {
 public:
  int maxJumps(vector<string> board) {
    ...
    set<int> hy;
    queue<pair<int,pair<int,set<int> > > > q; // mv,loc,history
    q.push(make_pair(0,make_pair(0,hy)));
    int mv=0;
    while(!q.empty()){
      pair<int,pair<int,set<int> > > t=q.front(); q.pop();
      mv=t.first;
      int loc=t.second.first;
      set<int> hy=t.second.second;
      //printf("move #%d, %d=(%d,%d)\n", mv, loc,x_of(loc),y_of(loc));
      ...
      if(x-a>=0 && board[y][x-a]!='H') q.push(make_pair(mv+1,make_pair(xy(x-a,y),hy)));
      if(x+a<w && board[y][x+a]!='H') q.push(make_pair(mv+1,make_pair(xy(x+a,y),hy)));
      if(y-a>=0 && board[y-a][x]!='H') q.push(make_pair(mv+1,make_pair(xy(x,y-a),hy)));
      if(y+a<h && board[y+a][x]!='H') q.push(make_pair(mv+1,make_pair(xy(x,y+a),hy)));
    }
    return mv+1;
  }
};
