...
class BedroomFloor {
 public:
  ...
  void check(vector<long long>&cnt, int x1,int y1,int x2,int y2){
    ...
    mycnt[0]=0;
    //printf("(%d,%d) - (%d,%d) : ", x1,y1, x2,y2); cout << mycnt << endl;
    for(int i=1;i<=5;i++) cnt[i]+=mycnt[i];
  }
  long long numberOfSticks(int x1, int y1, int x2, int y2) {
    ...
    if (xa<xb) {
      if (ya>yb) {
        //cout << "1"<<endl;
        check(cnt, 0,y1%10, 10,y2%10);
      } else {
        //cout << "2"<<endl;
        if(y1<ya) check(cnt, 0,y1%10, 10,10);
        check(cnt, 0,0, 10,y2%10);
      }
    }
    //cout << "D"<<endl;
    if (ya<yb) {
      if (xa>xb) {
        //cout << "3"<<endl;
        check(cnt, x1%10,0, x2%10,10);
      } else {
        //cout << "4"<<endl;
        if(x1<xa) check(cnt, x1%10,0, 10,10);
        check(cnt, 0,0, x2%10,10);
      }
    }
