 ...
 public:
  int calculateTime(vector <string> favorites) {
    ...
    rep(i,n){
      ...
      tr(f,it) fav[i][*it]=true;//fav[i] |= (1LL << *it);
    }
    //cout << "fav: " << fav << endl;
    set<pair<ll,ll> > s;
    priority_queue<vector<ll> > pq; // -sc, ofs, lastcmd, eaten
    vector<ll> start(3,0);
    pq.push(start); //make_pair(0,make_pair(0,make_pair(0,0LL))));
    while(!pq.empty()){
      vector<ll> t=pq.top();
      if (found(s,make_pair(t[1],t[2]))) { pq.pop(); continue; }
      pq.pop();

      //cout << t << endl;
      ll sc=-t[0], last=t[1]&3LL, ofs=(t[1]>>2), etn=t[2];
      //if (ofs < 0) ofs += n;
      //printf("sc:%lld, last:%lld, ofs:%lld, etn:%llx\n", sc,last,ofs,etn);
      s.insert(make_pair(t[1],t[2]));

      // rep(i,n) if ((etn[i] & fav[i]) < fav[i]) finis=false;
      if (etn == ful) return sc;
