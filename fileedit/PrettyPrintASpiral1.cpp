...
class PrettyPrintASpiral {
  ...
  int rows, cols, maxn, maxk, w;
  char *buf;

  inline char *sput(char *tail, int n){
    rep(i,maxk){
      *tail-- = (n>0)? ('0'+(n%10)) : ' ';
      n/=10;
    }
  ...
  string seg(int row, int col1, int col2){
    char *p = &buf[w*cols-1];
    int n;
    if (row==0) {
      // 0:1, +i: (2i-1)^2+i = 4i2-3i+1
      //      -i: 4i2-i+1
      for(int c=col2;c>=col1;c--){
        n = (c<0) ? 4*c*c-c+1 : 4*c*c-3*c+1;
        *p-- = (c==col2)? '\0':' ';
        p=sput(p,n);
      }
      //    } else if (row<0) {
 ...
 public:
  vector<string> getWindow(int row1, int col1, int row2, int col2) {
    rows = 1+row2-row1;
    cols = 1+col2-col1;
    maxn = max( max( num(col1,row1), num(col1,row2) ),
                max( num(col2,row1), num(col2,row2) ) );
    maxk = k(maxn); w=maxk+1;
    buf = (char*)malloc(w*cols);
    
    vector<string> ans(rows);
    for(int r=row1;r<=row2;r++) ans[r-row1] = seg(r, col1,col2);
