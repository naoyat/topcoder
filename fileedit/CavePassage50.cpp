...
public:
  int minimalTime(vector<int> travelersWeights,
	...
	//	vector<bool> possible(patterns, false);
	set<int> possibles;
	vector<int> weight(patterns);
	vector<int> minute(patterns);
...
/*
	for (int m=1; m<=all_mask; m++) {
	  if (possible[m]) distance_to_fix[AWAY][m] = minute[m];
	}
*/
// END CUT HERE
	tr(possibles,it) {
...
// BEGIN CUT HERE
	/*
	if (distance_to_fix[AWAY][goal] < INT_MAX) {
	  // cout << "一発!" << distance_to_fix[AWAY][goal] << endl;
	  return distance_to_fix[AWAY][goal];
	}
	*/
// END CUT HERE
...
// BEGIN CUT HERE
	  // 対岸から
	  // cout << "AWAY: " << endl;
	  // cout << "  " << distance_fixed[AWAY] << endl;
	  // cout << "  " << distance_to_fix[AWAY] << endl;
// END CUT HERE
	  int away_min = INT_MAX, away_at = -1;
	  for (int j=1; j<=all_mask; j++) {
		if (distance_to_fix[AWAY][j] < away_min) {
		  away_min = distance_to_fix[AWAY][j];
		  away_at = j;
		}
	  }
	  if (away_at > 0) {
		...
		for (int j=1; j<=all_mask; j++) {
		  if (distance_to_fix[AWAY][j] == away_min) {
			// printf("<away> fixing %x: %d\n", j, away_min);
			distance_fixed[AWAY][j] = away_min;
			distance_to_fix[AWAY][j] = INT_MAX;
			if (j == goal) return away_min;
...
// BEGIN CUT HERE
			...
			for (int m=1; m<all_mask; m++) {
			  ...
			  if ((j & m) == m) {
				int d = away_min + minute[m];
				int home_at = j - m;
				if (distance_fixed[HOME][home_at] < INT_MAX) continue;
				// printf("    %d→%d << %d %d\n", all_mask - j, all_mask - home_at, m, j);
				if (d < distance_to_fix[HOME][home_at]) {
				  distance_to_fix[HOME][home_at] = d;
				}
			  }
			}
			*/
// END CUT HERE
			tr(possibles,it) {
			  ...
			  if ((j & m) == m) {
				int d = away_min + minute[m];
				int home_at = j - m;
				if (home_at == 0) continue;
				if (distance_fixed[HOME][home_at] < INT_MAX) {
				  if (d < distance_fixed[HOME][home_at]) {
					printf("!!! d(=%d) < distance_fixed[HOME][%d]=%d\n",
						   d, home_at, distance_fixed[HOME][home_at]);
				  }
				  continue;
				}
				// printf("    %d→%d << %d %d\n", all_mask - j, all_mask - home_at, m, j);
...
// BEGIN CUT HERE
		/*
		distance_fixed[AWAY][away_at] = away_min;
		distance_to_fix[AWAY][away_at] = INT_MAX;
		if (away_at == goal) return away_min;
...
// BEGIN CUT HERE
	  /*
	  if (distance_fixed[AWAY][goal] < INT_MAX) return distance_fixed[AWAY][goal];
	  if (distance_to_fix[AWAY][goal] < INT_MAX) {
		cout << "(possibility) " << distance_to_fix[AWAY][goal] << endl;
	  }
	  */
	  
	  // こちらから
	  // cout << "HOME: " << endl;
	  // cout << "  " << distance_fixed[HOME] << endl;
	  // cout << "  " << distance_to_fix[HOME] << endl;
// END CUT HERE
	  int home_min = INT_MAX, home_at = -1;
	  for (int i=1; i<all_mask; i++) {
		if (distance_to_fix[HOME][i] < home_min) {
		  home_min = distance_to_fix[HOME][i];
		  home_at = i;
		}
	  }
	  if (home_at > 0) {
		...
		for (int i=1; i<all_mask; i++) {
		  if (distance_to_fix[HOME][i] == home_min) {
			// printf("<home> fixing %x: %d\n", i, home_min);
			distance_fixed[HOME][i] = home_min;
			distance_to_fix[HOME][i] = INT_MAX;
// BEGIN CUT HERE
			// HOME > AWAY伝播
			/*
			for (int m=1; m<all_mask; m++) {
			  if (!possible[m]) continue;
			  // if (((all_mask - i) & m) == (all_mask - i)) {
			  if ((i & m) == 0) {
				int d = home_min + minute[m];
				int away_at = i + m;
				if (distance_fixed[AWAY][away_at] < INT_MAX) continue;
				// printf("    %d %d >> %d → %d\n", all_mask - i, m, i, away_at);
				if (d < distance_to_fix[AWAY][away_at]) {
				  distance_to_fix[AWAY][away_at] = d;
				}
			  }
			}
			*/
// END CUT HERE
			tr(possibles,it) {
			  ...
			  if ((i & m) == 0) {
				int d = home_min + minute[m];
				int away_at = i + m;
				if (distance_fixed[AWAY][away_at] < INT_MAX) {
				  if (d < distance_fixed[AWAY][away_at]) {
					printf("!!! d(=%d) < distance_fixed[AWAY][%d]=%d\n",
						   d, away_at, distance_fixed[AWAY][away_at]);
				  }
				  continue;
				}
				// printf("    %d %d >> %d → %d\n", all_mask - i, m, i, away_at);
	  ...
	  if (away_at == -1 && home_at == -1) break;
	}
// BEGIN CUT HERE
	/*
	if (distance_fixed[AWAY][goal] < INT_MAX) return distance_fixed[AWAY][goal];
	if (distance_to_fix[AWAY][goal] < INT_MAX) return distance_to_fix[AWAY][goal];
	return -1;
	*/
// END CUT HERE
	return (distance_to_fix[AWAY][goal] < INT_MAX)? distance_to_fix[AWAY][goal] : -1;
  }
