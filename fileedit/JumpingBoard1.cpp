 ...
 public:
  int maxJumps(vector<string> board) {
    ...
    while(!q.empty()){
      ...
      for(int i=x-1,c=0;i>=0;i--){
        if(board[y][i]!='H') c++;
        if(c==a){
          q.push(make_pair(mv+1,xy(i,y))); break;
        }
      }
      for(int i=x+1,c=0;i<w;i++){
        if(board[y][i]!='H') c++;
        if(c==a){
          q.push(make_pair(mv+1,xy(i,y))); break;
        }
      }
      for(int i=y-1,c=0;i>=0;i--){
        if(board[i][x]!='H') c++;
        if(c==a){
          q.push(make_pair(mv+1,xy(x,i))); break;
        }
      }
      for(int i=y+1,c=0;i<h;i++){
        if(board[i][x]!='H') c++;
        if(c==a){
          q.push(make_pair(mv+1,xy(x,i))); break;
        }
