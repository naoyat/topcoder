...
public:
  ...
  double turn(int n,double r,int x,int y,vector<int> h) {
	...
	h[n] = 29*y+x;
	double total = 0.0;
	if (e_ > 0) total += turn(n-1,r*e_*0.01,x+1,y,h);
	if (w_ > 0) total += turn(n-1,r*w_*0.01,x-1,y,h);
	if (n_ > 0) total += turn(n-1,r*n_*0.01,x,y-1,h);
	if (s_ > 0) total += turn(n-1,r*s_*0.01,x,y+1,h);
	return total;
  }
