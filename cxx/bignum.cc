...
class bignum {
 ...
 public:
  ...
  bignum(int sg,const vector<unsigned int> &vals,int radix=0){set(sg,vals,radix);}
  ...
  void set(int sg,const vector<unsigned int> &vals,int radix=0);
 ...
 private:
  ...
  char enc(int n){ return n<10 ? '0'+n : 'A'+(n-10); }
  int dec(int ch){ return ch<='9' ? ch-'0' : 10+ch-'A'; }
 ...
 public:
  ...
  bool operator==(const bignum& rop){ return cmp(rop)==0; }
  bool operator!=(const bignum& rop){ return cmp(rop)!=0; }
  bool operator>(const bignum& rop){ return cmp(rop)>0; }
  bool operator>=(const bignum& rop){ return cmp(rop)>=0; }
  bool operator<(const bignum& rop){ return cmp(rop)<0; }
...
void bignum::set(int sg,const vector<unsigned int> &vals,int radix){
  if (radix == 0) {
    size=1;
...
void bignum::operator+=(const bignum &n){
  if(n.is_zero()) return;
  if(sign == n.sign) {
    if(size < n.size) values.resize(size=n.size);
    ...
    if(cmp_abs(n) >= 0) {
      ...
      sign = -sign;
      for(int i=0;i<n.size;i++) values[i] = n.values[i] - values[i];
    }
    normalize();
  }
}
